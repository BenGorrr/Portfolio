    ...
    if len(steamid64) == 1: #if only one id is passed in, just set the id to it
        ...
        for id in steamid64: #Loop tru each id and concat them into comma seperated string which the API require to request multiple ids
            if id == steamid64[0]:
                ...
                finalSteamid = finalSteamid + id + ','
    url = 'http://api.steampowered.com/ISteamUser/GetPlayerBans/v1/?key=1A9A1AF309EA19B10FB5B3172C0FEDA8&steamids=' + finalSteamid
...
def get_Display_Name(steamid64):
    url = 'http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=1A9A1AF309EA19B10FB5B3172C0FEDA8&steamids=' + steamid64
    req = requests.get(url)
    req_json = req.json()
...
def get_Games_Owned(steamid64):
    url = 'http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=1A9A1AF309EA19B10FB5B3172C0FEDA8&steamid=' + steamid64
    req = requests.get(url)
    req_json = req.json()
...
def get_Game_Stats(appid, steamid64):
    url = 'http://api.steampowered.com/ISteamUserStats/GetUserStatsForGame/v0002/?appid='+ appid + '&key=1A9A1AF309EA19B10FB5B3172C0FEDA8&steamid=' + steamid64
    req = requests.get(url)
    req_json = req.json()
    #write_Json('data.txt', req_json)
